...
if (!function_exists("console_log")) {
    function console_log($data) {
        Log::debug($data); // Imprimiendo en log de Laravel
    }
}
...
if (!function_exists("to_numeric_text")) {
    function to_numeric_text($number): ?string {
        if (is_null($number)) { 
            return null; // Número indefinido
        } 

        if ($number == 0) { 
            return "CERO"; // Numero es '0'
        } 
        ...
        while ($end_index > 0) {
            $start_index = $end_index - 3; // Definiendo posición de inicio

            if ($start_index < 0) { 
                $start_index = 0; // Controlando el desbordamiento
            }
                ...
                if (!is_null($description)) {
                    if ($index > 0) {
                        if ((int) $digits_three > 1) {
                            ...
                            if (($index % 2) !== 0) { 
                                $description = $quantites[$index]; // Número es unidad de mil
                            } else { 
                    ...
                    $result = "{$description} {$result}"; // Adjuntando
                }
            }

            $index++; $end_index = $start_index; // Reconfigurando variables
        }

        return trim($result); // Retornando descripción del número
    }
}

if (!function_exists("to_three_digits_text")) {
    function to_three_digits_text($digits_three): ?string {
        $number = (string) $digits_three; // Numero de trés digitos
        
        if ((strlen($number) > 3) || (strlen($number) < 1)) { 
            return null; // Formato incorrecto
        } 
        
        $result         = ""; // Expresión del número
        ...
        if ($hundred > 0) {
            ...
            if (($hundred == 1 && $ten != 0)) {
                $hundred_end = "TO"; // Agregar sufijo de 100
            }
        ...
        if ($ten > 0) {
            if (isset($digits[$ten])) {
                $result = "$result{$digits[$ten]}"; // Múltiplo de 10
            } else {
        ...
        return trim($result); // Retornando expresión del número
    } 
}
